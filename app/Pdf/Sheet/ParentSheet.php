...
class ParentSheet extends Fpdf
{
	...
	public function Header()
	{
	    ...
	    // Movernos a la derecha
	    $this->Cell(25, 4, '', 0,0);
	    $this->Cell(40, 4, (!is_null($this->group->director()->first())) ? "DIR. GRUPO: ".$this->group->director()->first()->manager->fullName : "DIR. GRUPO", 0, 0, 'L');
	    // Título
	...
	private function subheader()
	{
		...
		$this->Cell($this->_with_TC, '5', $this->hideTilde('Teléfono Acudiente'), 1, 0, 'C', true);
	...
	public function create()
	{
		...
		foreach($this->data as $key => $student)
		{
			$this->Cell($this->_with_FC, '6', ($key < 9) ? "0".($key+1) : ($key+1) , 1, 0, 'C');
			$this->Cell($this->_with_SC, '6', strtoupper($this->hideTilde($student->fullNameInverse)), 1, 0, 'L');

			$this->Cell($this->_with_TC, '6', $this->getPhoneParent($student->family), 1, 0, 'C');
	...
	public function getPhoneParent($parents = array())
	{
		$phones = '';
		...
		if(!is_null($parents))
		{
			...
			{
				$fixed = $parent->address->phone;
				$mobil = $parent->address->mobil;

				$phones .= (!is_null($fixed)) ? "{$fixed} - " : "";
				$phones .= (!is_null($mobil)) ? "{$mobil} - " : "";
			}
		}
